    ...
    public class Maze
    {
        ...
        public IEnumerable<RemoveWallAction> GetNeighbours(Point p)
        {
            if (p.X > 0)
            {
                ...
                {
                    Neighbour = new Point(p.X - 1, p.Y), Wall = CellState.Left
                };
            }
            if (p.Y > 0)
            {
                yield return new RemoveWallAction
                {
                    Neighbour = new Point(p.X, p.Y - 1), Wall = CellState.Top
                };
            }
            if (p.X < _width - 1)
            {
                yield return new RemoveWallAction
                {
                    Neighbour = new Point(p.X + 1, p.Y), Wall = CellState.Right
                };
            }
            if (p.Y < _height - 1)
            {
                yield return new RemoveWallAction
                {
                    Neighbour = new Point(p.X, p.Y + 1), Wall = CellState.Bottom
                };
            }
        }
